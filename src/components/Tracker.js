      ...
      <>
        {activities.map((activity, i) =>
          ...
          <div className="activity-container" key={activity.id}>
            ...
            <div className="date-box">
              <div className="day-box">
                {this.getDays(activity.daysInMonth).map((value) =>
                  <p
                    key={value}
                    className={`day-box ${this.isActiveDay(activity, value) ? 'active' : ''}`}
                    onClick={() => this.props.setActivityDay(activity.id, value)}
                  >
                    {value}
                  </p>
                )}
            ...
            <div>
              <button className="delete-btn" onClick={() => this.props.handleDelete(activity.id)}>
                X
              </button>
            </div>
          </div>
